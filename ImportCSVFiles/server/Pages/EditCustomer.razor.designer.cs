...
namespace ImportCsvFiles.Pages
{
    public partial class EditCustomerComponent : ComponentBase
    {
        ...
        {
            var csvImportExampleGetCustomerByCustomerIdResult = await CsvImportExample.GetCustomerByCustomerId(CustomerID);
            customer = csvImportExampleGetCustomerByCustomerIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(ImportCsvFiles.Models.CsvImportExample.Customer args)
        {
            try
            {
                var csvImportExampleUpdateCustomerResult = await CsvImportExample.UpdateCustomer(CustomerID, customer);
                DialogService.Close(customer);
            }
            catch (System.Exception csvImportExampleUpdateCustomerException)
            {
